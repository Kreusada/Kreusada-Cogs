...
class TextManipulator(commands.Cog):
    ...
    @commands.group()
    async def tm(self, ctx: commands.Context):
        """Base command for manipulating with text."""

    @tm.command(name="charcount")
    async def tm_charcount(self, ctx: commands.Context, include_spaces: Optional[bool] = True, *, text: str):
    ...
    @tm.command(name="wordcount")
    async def tm_wordcount(self, ctx: commands.Context, *, text: str):
    ...
    @tm.command(name="occurance")
    async def tm_occurance(self, ctx: commands.Context, check: str, *, text: str):
    ...
    @tm.command(name="upper")
    async def tm_upper(self, ctx: commands.Context, *, text: str):
        """Convert the text to uppercase."""
        await ctx.send(box(text.upper()))

    @tm.command(name="lower")
    async def tm_lower(self, ctx: commands.Context, *, text: str):
        """Convert the text to lowercase."""
        await ctx.send(box(text.lower()))

    @tm.command(name="title")
    async def tm_title(self, ctx: commands.Context, *, text: str):
        """Convert the text to titlecase."""
        await ctx.send(box(text.title()))

    @tm.command(name="snake")
    async def tm_snake(self, ctx: commands.Context, *, text: str):
        """Convert all spaces to underscores."""
        snake = lambda x: x.replace(" ", "_")
        await ctx.send(box(snake(text)))

    @tm.command(name="alternating")
    async def tm_alternating(self, ctx: commands.Context, *, text: str):
    ...
    @tm.command(name="replace")
    async def tm_replace(
        self, 
    ...
    @tm.command(name="squash")
    async def tm_squash(self, ctx: commands.Context, *, text: str):
        """Squash all the words into one."""
        await ctx.send(box(text.replace(" ", "")))

    @tm.command(name="remove")
    async def tm_remove(self, ctx: commands.Context, remove: str, *, text: str):
        """Remove something from the text."""
        await ctx.send(box(text.replace(remove, "")))

    @tm.command(name="trim", aliases=["strip"])
    async def tm_trim(self, ctx: commands.Context, trimmer: Optional[str] = " ", *, text: str):
        """Trim the outskirts of the text."""
        await ctx.send(box(text.strip(trimmer)))

    @tm.command(name="shuffle", aliases=["jumble"])
    async def tm_shuffle(self, ctx: commands.Context, *, text: str):
        """Completely shuffle the text."""
        data = text.split()
        random.shuffle(data)
        await ctx.send(box(" ".join(data)))

    @tm.command(name="reverse")
    async def tm_reverse(self, ctx: commands.Context, *, text: str):
        """Reverse the text."""
