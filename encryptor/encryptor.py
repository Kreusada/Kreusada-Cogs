...
class Encryptor(commands.Cog):
    ...
    async def password_generate_complex(self, ctx):
        ...
        await ctx.send("".join(random.choice(string.ascii_letters[:94]) for i in range(random.randint(20,35))))
    ...
    async def password_generate_strong(self, ctx, delimeter: str = ""):
        ...
        * ``<delimeter>``: The character used to seperate each random word. Defaults to "-"
        ...
        rc = random.choice
        rr = random.randint
        await ctx.send(d.join(rc(random_words).capitalize() for i in range(3)) + f"{d}{rr(1,1000)}")
    ...
    async def password_strength(self, ctx, password: str):
        ...
        await ctx.send(f"**Strength rating: {round(converter * 100)}%** {emoji}\n{cf.quote(text)}")
